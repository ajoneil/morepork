// morepork-gateboy: Adapter that uses GateBoy (from metroboy) to produce
// .morepork files.
//
...
// per instruction boundary, matching the output format of the other morepork
// adapters.
//
...
//
// Usage:
//   morepork-gateboy --rom test.gb --profile cpu_basic.toml [--output trace.morepork]
//
// Build:
...
static Profile load_profile(const std::string &path) {
    MoreporkProfile *p = morepork_profile_load(path.c_str());
    if (!p) {
    ...
    Profile prof;
    prof.name = morepork_profile_name(p);
    prof.trigger = morepork_profile_trigger(p);
...
// --- Trace writer (FFI) ---
static MoreporkWriter *g_writer = nullptr;
...
static void emit_entry(GateBoy &gb) {
    ...
    for (size_t i = 0; i < g_emitters.size(); i++) {
        ...
        case FieldEmitter::CPU_REG8:
            morepork_writer_set_u8(g_writer, col, read_cpu_reg8(reg, em.name));
            break;
        case FieldEmitter::CPU_REG16:
            morepork_writer_set_u16(g_writer, col, read_cpu_reg16(reg, em.name));
            break;
        case FieldEmitter::CPU_IME:
            morepork_writer_set_bool(g_writer, col, reg.ime);
            break;
        case FieldEmitter::IO_READ:
            morepork_writer_set_u8(g_writer, col, read_reg(gb, em.io_addr));
            break;
        case FieldEmitter::PIX:
            if (g_pix_buf.empty()) {
                morepork_writer_set_null(g_writer, col);
            } else {
                morepork_writer_set_str(g_writer, col,
                                       g_pix_buf.c_str(), g_pix_buf.size());
            }
            g_pix_buf.clear();
            break;
        case FieldEmitter::PPU_U8:
            morepork_writer_set_u8(g_writer, col, em.read_u8(gb));
            break;
        case FieldEmitter::PPU_U16:
            morepork_writer_set_u16(g_writer, col, em.read_u16(gb));
            break;
        case FieldEmitter::PPU_BOOL:
            morepork_writer_set_bool(g_writer, col, em.read_bool(gb));
            break;
        }
    }
    ...
    if (g_writer_vram_addr_col >= 0) {
        if (g_vram_write_addr != 0) {
            morepork_writer_set_u16(g_writer, g_writer_vram_addr_col, g_vram_write_addr);
            morepork_writer_set_u8(g_writer, g_writer_vram_data_col, g_vram_write_data);
        } else {
            morepork_writer_set_null(g_writer, g_writer_vram_addr_col);
            morepork_writer_set_null(g_writer, g_writer_vram_data_col);
        }
    }
    g_vram_write_addr = 0;
    g_vram_write_data = 0;
    ...
    if (g_writer_apu_write_addr_col >= 0) {
        if (g_apu_write_addr != 0) {
            morepork_writer_set_u16(g_writer, g_writer_apu_write_addr_col, g_apu_write_addr);
            morepork_writer_set_u8(g_writer, g_writer_apu_write_data_col, g_apu_write_data);
        } else {
            morepork_writer_set_null(g_writer, g_writer_apu_write_addr_col);
            morepork_writer_set_null(g_writer, g_writer_apu_write_data_col);
        }
    }
    g_apu_write_addr = 0;
    g_apu_write_data = 0;
...
int main(int argc, char *argv[]) {
...

    {
        ...
        for (size_t i = 0; i < g_emitters.size(); i++) {
            g_writer_cols[i] = morepork_writer_find_field(
                g_writer, g_emitters[i].name.c_str());
        }
        g_writer_vram_addr_col = morepork_writer_find_field(g_writer, "vram_addr");
        g_writer_vram_data_col = morepork_writer_find_field(g_writer, "vram_data");
        g_has_vram = (g_writer_vram_addr_col >= 0);
        g_writer_apu_write_addr_col = morepork_writer_find_field(g_writer, "apu_write_addr");
        g_writer_apu_write_data_col = morepork_writer_find_field(g_writer, "apu_write_data");
