// morepork-sameboy: Adapter that uses SameBoy to produce .morepork files.
//
...
//
// Usage:
//   morepork-sameboy --rom test.gb --profile cpu_basic.toml --output trace.morepork
//
// Build:
...
static void emit_entry(GB_gameboy_t *gb, uint16_t address) {
    ...
    for (size_t i = 0; i < g_emitters.size(); i++) {
        ...
        case FieldEmitter::REGISTER_8:
            morepork_writer_set_u8(g_writer, col, read_reg(gb, em.reg));
            break;
        case FieldEmitter::REGISTER_16:
            if (em.reg == RegisterField::PC)
                ...
                morepork_writer_set_u16(g_writer, col,
                                       g_tcycle_mode ? GB_get_registers(gb)->pc : address);
            else
                morepork_writer_set_u16(g_writer, col, read_reg(gb, em.reg));
            break;
        case FieldEmitter::OP_ADDR:
            morepork_writer_set_u16(g_writer, col, address);
            break;
        case FieldEmitter::IO_READ:
            morepork_writer_set_u8(g_writer, col, GB_safe_read_memory(gb, em.io_addr));
            break;
        case FieldEmitter::IME:
            morepork_writer_set_bool(g_writer, col, gb->ime);
            break;
