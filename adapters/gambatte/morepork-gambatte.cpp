// morepork-gambatte: Adapter that uses libgambatte to produce .morepork files.
//
...
//
// Usage:
//   morepork-gambatte --rom test.gb --profile cpu_basic.toml --output trace.morepork
//
// Build:
...
// profile requests them under [fields.extensions] gambatte. The test writes
...
static std::string gbmicrotest_extension_json(const std::vector<Emitter> &emitters) {
    ...
    for (const auto &em : emitters) {
        ...
        if (!first) json += ",";
        first = false;
        json += "\"" + em.name + "\":{\"type\":\"u8\",\"source\":\"gambatte\","
...
static Profile load_profile(const std::string &path) {
    ...
    size_t next = morepork_profile_num_extensions(p, "gambatte");
    for (size_t i = 0; i < next; i++)
        prof.fields.push_back(morepork_profile_extension_name(p, "gambatte", i));
...
static void emit_entry(int *r) {
    ...
    for (size_t i = 0; i < g_emitters.size(); i++) {
        ...
        case FieldEmitter::CALLBACK_8:
            morepork_writer_set_u8(g_writer, col, r[em.cb_index] & 0xFF);
            break;
        case FieldEmitter::CALLBACK_16:
            morepork_writer_set_u16(g_writer, col, r[em.cb_index] & 0xFFFF);
            break;
        case FieldEmitter::IO_READ:
            if (g_io_cache_valid) {
                morepork_writer_set_u8(g_writer, col, g_io_cache[em.io_addr]);
            } else {
                morepork_writer_set_u8(g_writer, col, io_now[em.io_addr]);
            }
            break;
...
int main(int argc, char *argv[]) {
    ...
    header_json += "]," + gbmicrotest_extension_json(g_emitters) + "\"trigger\":\"instruction\"}";
    ...
    std::fprintf(stderr, "Output: morepork (native format)\n");
