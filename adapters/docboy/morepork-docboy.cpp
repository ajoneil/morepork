// morepork-docboy: Adapter that uses DocBoy to produce .morepork files.
//
...
//
// Usage:
//   morepork-docboy --rom test.gb --profile cpu_basic.toml --output trace.morepork
//
// Build:
...
// profile requests them under [fields.extensions] docboy. The test writes
...
static std::string gbmicrotest_extension_json(const std::vector<Emitter> &emitters) {
    ...
    for (const auto &em : emitters) {
        ...
        if (!first) json += ",";
        first = false;
        json += "\"" + em.name + "\":{\"type\":\"u8\",\"source\":\"docboy\","
...
static Profile load_profile(const std::string &path) {
    ...
    size_t next = morepork_profile_num_extensions(p, "docboy");
    for (size_t i = 0; i < next; i++)
        prof.fields.push_back(morepork_profile_extension_name(p, "docboy", i));
...
static void emit_entry() {
    ...
    for (size_t i = 0; i < g_emitters.size(); i++) {
        ...
        case FieldEmitter::CPU_REG8:
            morepork_writer_set_u8(g_writer, col, read_cpu_reg8(em.name));
            break;
        case FieldEmitter::CPU_REG16:
            morepork_writer_set_u16(g_writer, col, read_cpu_reg16(em.name));
            break;
        case FieldEmitter::IO_READ:
            morepork_writer_set_u8(g_writer, col, g_debugger->read_memory(em.io_addr));
            break;
        case FieldEmitter::IME:
            morepork_writer_set_bool(g_writer, col, g_cpu_snap.ime);
            break;
...
int main(int argc, char *argv[]) {
    ...
    if (rom_path.empty() || profile_path.empty() || output_path.empty()) {
        std::fprintf(stderr,
            "Usage: morepork-docboy --rom <file.gb> --profile <profile.toml> --output <path> [options]\n");
        return 1;
    }
    ...
    header_json += "]," + gbmicrotest_extension_json(g_emitters) + "\"trigger\":\"tcycle\"}";
