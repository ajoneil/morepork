//! Convert from any supported input format to the native `.morepork` format.
//!
//! Supports: JSONL and existing `.morepork` files.
//! Produces a `MoreporkStore` either from a file or from in-memory bytes.
...
use super::read::MoreporkStore;
use super::write::MoreporkWriter;

/// Convert JSONL bytes to a `MoreporkStore` (in-memory).
/// Writes to a temp file, then loads it back.
pub fn jsonl_to_store(data: &[u8]) -> Result<MoreporkStore> {
...
/// Convert a JSONL file to a `MoreporkStore`.
pub fn jsonl_file_to_store(path: &std::path::Path) -> Result<MoreporkStore> {
    let reader = crate::reader::JsonlReader::open(path)?;
    let header = reader.header().clone();
    reader_to_store(header, reader)
}

/// Convert any `TraceEntry` iterator + header into a `MoreporkStore`.
fn reader_to_store(
    mut header: TraceHeader,
    entries: impl Iterator<Item = Result<TraceEntry>>,
) -> Result<MoreporkStore> {
...

    {
        ...
        for result in entries {
            ...
            let frame_boundary = entry
                ...
                .unwrap_or(false);
            write_entry_to_morepork(&mut writer, &entry, &header)?;
            writer.finish_entry()?;
            if frame_boundary {
    ...
    MoreporkStore::from_bytes(&data)
}

/// Write a single `TraceEntry` to a `MoreporkWriter`, handling types and nullability.
pub fn write_entry_to_morepork(
    writer: &mut MoreporkWriter,
    entry: &TraceEntry,
    header: &TraceHeader,
) -> Result<()> {
