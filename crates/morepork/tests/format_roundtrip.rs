...
fn test_basic_roundtrip() {
    ...
    let path = dir.path().join("test.morepork");
...
fn test_large_chunk_boundary() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("large.morepork");
...
fn test_framebuffer() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("fb.morepork");
...
fn test_extension_fields_roundtrip() {
    use morepork::header::ExtensionField;
...
fn test_empty_trace() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("empty.morepork");
    ...
    // Write empty trace
    {
        let w = MoreporkWriter::create(&path, &header, &groups).unwrap();
        w.finish().unwrap();
    }
...
fn test_header_self_describing_on_write() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("selfdesc.morepork");
...
fn test_writer_derives_groups_from_defs_when_none_given() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("nogroups.morepork");

    let header = test_header();
    {
        let mut w = MoreporkWriter::create(&path, &header, &[]).unwrap();
        w.set_u16(0, 0x0150);
        w.set_u16(1, 0xFFFE);
        for col in 2..13 {
            w.set_u8(col, 0);
...
fn rejects_headers_without_field_metadata() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("legacy.morepork");
    let header = TraceHeader {
        _header: true,
        format_version: "0.1.0".into(),
        emulator: "test".into(),
    ...
    {
        let w = MoreporkWriter::create(&path, &header, &[]).unwrap();
        w.finish().unwrap();
    }
    let data = std::fs::read(&path).unwrap();
    ...
    match MoreporkStore::from_bytes(&out) {
