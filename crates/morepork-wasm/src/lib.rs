...
pub struct TraceStore {
    store: Box<dyn morepork::store::TraceStore>,
    rom: Option<Vec<u8>>,
...
impl TraceStore {
    ...
    /// Supports native .morepork and JSONL.
    #[wasm_bindgen(constructor)]
    pub fn from_bytes(data: &[u8]) -> Result<TraceStore, JsError> {
        let store = morepork::store::open_trace_store_from_bytes(data)
        ...
        let is_pixel_stream = |name: &str| {
            header.field_def(name).is_some_and(|def| {
                def.layer.as_deref() == Some("output")
                    && def.field_type == morepork::profile::FieldType::Str
            })
        };
    ...
    pub fn has_indexed_frames(&self) -> bool {
        self.store.header().pix_format == morepork::PixFormat::Indexed8
    }
    ...
    pub fn indexed_frame(&self, frame_index: usize) -> Result<JsValue, JsError> {
        ...
        let frame = match morepork::snapshot::IndexedFrame::from_bytes(&payload) {
            Some(f) => f,
            None => return Ok(JsValue::NULL),
        };
    ...
    pub fn has_per_entry_pixels(&self) -> bool {
        ...
        self.store.header().trigger == morepork::header::Trigger::Tcycle
    }
    ...
    pub fn render_frame(&self, frame_index: usize) -> Result<JsValue, JsError> {
        ...
        let frame = if let Some(ref ds_map) = self.downsample_map {
            let ds = morepork::downsample::DownsampledStore::from_map(&*self.store, ds_map.clone());
    ...
    pub fn pix_format(&self) -> String {
        match self.store.header().pix_format {
            morepork::PixFormat::Rgb555 => "rgb555".to_string(),
            morepork::PixFormat::Shade2 => "shade2".to_string(),
            morepork::PixFormat::Indexed8 => "indexed8".to_string(),
        }
    }
    ...
    pub fn pix_range(&self, start: usize, count: usize) -> Result<JsValue, JsError> {
        ...
        for i in start..end {
            ...
            let value = match format {
                ...
                morepork::PixFormat::Indexed8 => None,
            };
            if let Some(v) = value {
    ...
    pub fn diff_field(
        ...
        let indices = morepork::comparison::bulk_field_diff_indices(
    ...
    pub fn diff_stats_range(&self, other: &TraceStore, start: usize, end: usize) -> Result<JsValue, JsError> {
        ...
        for name in &fields {
            ...
            let count = morepork::comparison::bulk_field_diff_count(
    ...
    pub fn diff_all(&self, other: &TraceStore) -> Result<js_sys::Uint32Array, JsError> {
        ...
        for &name in &common_fields {
            ...
            let field_indices = morepork::comparison::bulk_field_diff_indices(
...
pub fn prepare_for_diff(a: &TraceStore, b: &TraceStore, sync: Option<String>) -> Result<JsValue, JsError> {
    let diff = morepork::comparison::TraceComparison::align(
